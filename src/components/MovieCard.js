...
const MovieCard = ({ poster_path, title, release_date, id, genre }) => {
  ...
  return (
    ...
    >
      <Link to={`/detail/${id}`}>
        <CardMedia component="img" image={poster_path} alt={title} />
        <CardContent>
      ...
      <CardActions sx={{ justifyContent: "space-around" }} disableSpacing>
        {favorites.favoriteFilms.some((film) => film.id === id) ? (
          ...
          <IconButton
            aria-label="share"
            onClick={() =>
              dispatch(addFavorites(id, title, genre, release_date))
            }
          >
            <FavoriteIcon />
          </IconButton>
        ...
        {seenlist.seenFilms.some((film) => film.id === id) ? (
          <IconButton
            aria-label="share"
            onClick={() => dispatch(deleteSeenlist(id))}
          >
          ...
          <IconButton
            aria-label="share"
            onClick={() =>
              dispatch(addSeenlist(id, title, genre, release_date))
            }
          >
            <BookmarkIcon />
          </IconButton>
